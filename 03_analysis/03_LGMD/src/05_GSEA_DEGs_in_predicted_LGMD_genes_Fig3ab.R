...
# DGA results
res <- read.csv(paste0(working_dir, '/03_analysis/03_LGMD/results/LGMD_DGEA_results.csv'))

# knockout results
ko_res_up <- read.csv(paste0(working_dir, '/03_analysis/03_LGMD/results/Genes_upregulating_LGMD_activity_Wilcoxon_results.csv'))
ko_res_dn <- read.csv(paste0(working_dir, '/03_analysis/03_LGMD/Genes_downregulating_LGMD_activity_Wilcoxon_results.csv'))
...
write.table(lgmd_up, paste0(working_dir, '/03_analysis/03_LGMD/results/LGMD_up.txt'), row.names=FALSE, col.names=FALSE,quote=FALSE)
write.table(lgmd_dn, paste0(working_dir, '/03_analysis/03_LGMD/results/LGMD_dn.txt'), row.names=FALSE, col.names=FALSE,quote=FALSE)
...
ggsave(paste0(working_dir, '/03_analysis/03_LGMD/figures/05-01_GSEA_HPO_LGMD_up.pdf'), p)

p <- enrichplot::gseaplot2(gsea_res_dn, geneSetID = 1, pvalue_table = TRUE, ES_geom = "line")
ggsave(paste0(working_dir, '/03_analysis/03_LGMD/figures/05-01_GSEA_HPO_LGMD_dn.pdf'), p)
...
write.csv(data.frame(c(up_lgmd_genes, dn_lgmd_genes)), paste0(working_dir, '/03_analysis/03_LGMD/results/LGMD_genes_in_leading_edge.csv'), row.names=FALSE, col.names=FALSE, quote=FALSE)
