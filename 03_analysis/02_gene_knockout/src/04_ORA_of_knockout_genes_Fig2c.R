...
get_genelist <- function(res, gene){
    res <- read.csv(paste0(working_dir, '/03_analysis/02_gene_knockout/results/differential_analysis/', gene, '_ranked_genes.csv'), sep=';')
...
ora_plots <- function(genelist, gene, name){
    res_go <- enrichGO(gene = genelist,
                    ...
                    ont = "BP", 
                    pvalueCutoff = 0.05)
    write.csv2(data.frame(res_go), paste0(working_dir, "/03_analysis/02_gene_knockout/results/ORA/GO_ORA_reconstruction_after_", gene, "_knockout.csv"), row.names=FALSE, quote = FALSE)
    go_bar <- overrep_bar_plot(res_go)
    ggsave(paste0(working_dir, "/03_analysis/02_gene_knockout/figures/GO_ORA_reconstruction_after_", gene, "_knockout.pdf"), go_bar, width=15, height=8)
}
