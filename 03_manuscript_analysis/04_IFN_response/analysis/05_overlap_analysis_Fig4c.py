...
res_plot = res.iloc[0:778,:]   # 778 is the rank from GSEA
...
color_map = dict(zip(list(np.arange(1,125)[::-1]), sns.color_palette("Reds", n_colors=124)))
...
plt.savefig(project_dir + '/03_manuscript_analysis/04_IFN_response/figures/05-01_ranking_influence_of_in_silico_stimulated_genes_on_IFN_signalling_leading_edge_Fig3c.pdf')
plt.clf()



# load descendant genes dict
with open(project_dir + "/ontologies/GO/graph/GO_symbol_trimmed_desc_genes.json") as jfile:
    desc_genes = json.load(jfile)
...
plt.savefig(project_dir + '/03_manuscript_analysis/04_IFN_response/figures/05-02_overlap_diff_genes_in_silico_stimulated_genes_leading_edge.pdf')
plt.clf()